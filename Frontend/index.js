...
//Using fetch()
async function makeRequestRepos(username) {
  try {
...
async function makeRequestRepoCommits(username, repo, commitsBox) {
  try {
    ...
    if (request.ok) {
      getResponseRepoCommits(request, commitsBox);
    } else {
      alertBoxShow(`${request.statusText} ${request.status}`, "normal");
    }
  } catch (error) {
...
// Using fetch()
function getResponseRepoCommits(req, commitsBox) {
  const reqJson = req.json();
  reqJson
    .then((data) => {
      returnRepoCommits(data, commitsBox);
    })
    .catch((err) => console.log(err));
}

function returnRepoCommits(commits, commitsBox) {
...
function returnRepo(username, repos) {
  ...
  count.textContent = `No. of repositories : ${repos.length}`;
